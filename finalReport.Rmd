...
But that's a free-form description and I don't think that's what we're talking about. Given this I do not see a variable that maps to this one.

KM: I took another pass through their codebook, and I think Question 4a maps onto the "Outcomes" variable. From the key,

> Outcomes: Thinking about how the events with regard to this incident will unfold in the future, how many diffe...-Number of alternatives

However, this variable does not exist in the `Study_2_updated.sav` raw data file. Thus, this is an:

INSUFFICIENT INFORMATION ERROR

KM note: the authors responded about the variable for Question 4a, saying:
...
d <- d_raw %>%
  select(ID, distance, issue, FAC1_1, flexible_n)
```
...
Eyeball checks for the mean composite scores in Figure 1.
...
We get two minor errors for the t-value and the $\eta^2$. Let's try running the ANOVA but filtering out the participants who are not part of the comparison of interest. 

```{r, eval = F}
c1_aov_2 <- ezANOVA(data = d_contrast %>% filter(contrast1 != 0), 
                  dv = FAC1_1, 
                  wid = ID, 
                  between = .(contrast1), 
                  type = 3)

c1_df_2 <- c1_aov_2$ANOVA$DFd
c1_f_2 <- c1_aov_2$ANOVA$F
c1_t_2 <- sqrt(c1_aov_2$ANOVA$F)
c1_p_2 <- c1_aov_2$ANOVA$p
c1_ges_2 <- c1_aov_2$ANOVA$ges

reportObject <- reproCheck(reportedValue = '113', 
                           obtainedValue = c1_df_2,
                           valueType = 'df')
reportObject <- reproCheck(reportedValue = '< .001', 
                           obtainedValue = c1_p_2,
                           valueType = 'p', eyeballCheck = TRUE)
reportObject <- reproCheck(reportedValue = '3.93', 
                           obtainedValue = c1_t_2,
                           valueType = 't')
reportObject <- reproCheck(reportedValue = '.12', 
                           obtainedValue = c1_ges_2,
                           valueType = 'pes')
```

In this version of the analysis, the degrees of freedom are way off becase not all of the data are being used and the $\eta^2$ value is off, but we get closer to reproducing the reported t-value. [We should try to reproduce in SPSS or ask for clarification about how the ANOVA was specified]
...
We generally reproduced the findings of the paper, despite some major numerical errors in small values. Our suspicion is that these are largely caused by SPSS/R differences; this hypothesis could be tested directly. We did have trouble reproducing the PCA to generate the wise reasoning composite scores from the raw data. We think this due to a missing variable for question 4A from Table 1 and 5 missing values in the raw data file that are not mentioned in the paper.
